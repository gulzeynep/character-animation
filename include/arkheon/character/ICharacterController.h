...
/* POD math primitives. Do not pass STL/Qt types across the ABI. */
...
typedef struct {
    uint8_t keys[256]; /* USB HID scancode -> held state */
    float mouse_dx;
    float mouse_dy;
    float look_yaw_rad;
    float look_pitch_rad;
    uint8_t mouse_buttons;   /* bit0=L, bit1=R, bit2=M */
    uint8_t hotkey_motion_a; /* edge-triggered */
...
typedef struct {
    ...
    int32_t (*get_object_aabb)(void* ctx, int32_t object_id, arkheon_vec3* out_min, arkheon_vec3* out_max);

    int32_t (*find_object_by_name)(void* ctx, const char* name);

    int32_t (*navmesh_query)(void* ctx, arkheon_vec3 from, arkheon_vec3 to, arkheon_vec3* out_path, int32_t max_pts);
...
ARKHEON_CHAR_EXPORT int32_t arkheon_character_tick(void* handle,
